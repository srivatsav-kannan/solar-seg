"""Context screen: frozen baseline training loop with only the model changed."""
...
def train(
    ...
    model = ContextUNet(width, depth).to(device)
    if initialize:
        ...
        model.initialize_base(initial["state_dict"])
    ...
    cfg = {
        ...
        "context_initialization": "zero output projection; exact parent function at initialization",
        "depth": depth,
