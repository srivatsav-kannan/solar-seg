...
Report PQ, segmentation quality SQ, recognition quality RQ, positive-pair IoU/Dice distributions, one-to-many/many-to-one counts, empty detections, and per-site/year/size behavior. Distribution plots that omit zero pairs are labeled accordingly; FP/FN counts show misses. Bootstrap **physical images**, retaining all annotators together. The 95% interval is descriptive and does not account for every source of temporal dependence. A block bootstrap is a useful stricter follow-up.
