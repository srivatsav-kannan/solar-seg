...
The frozen all-data recipe trained on all 707 approved observations, initializing the corresponding all-data baseline. Its checkpoint has no independent internal holdout score. All six submission gates passed; independent CPU inference and a fresh generated-notebook replay produced exactly the same CSV SHA-256 `9ae19115b1da6a997e3b494e714c631bba17acb711efc76c391cd61e7ffe4ec3`: 1,513 instances, 180 processed images, 178 nonempty and two empty. Submission **56055334** completed on 6 September 2026 with **public PQ 0.31**, up from the previous two entries at 0.30. The separate public Kaggle CPU notebook completed. It reproduced all 1,513 instance IDs, with one boundary pixel differing in one mask across CPU platforms (IoU 0.999684 for that mask); its CSV is therefore not byte-identical. See [cloud comparison](../reports/batch-dice-cloud-reproduction.json). [Gate evidence](../reports/batch-dice-gates.json), [fresh replay](../reports/batch-dice-replay.json), [server receipt](../reports/batch-dice-submission-record.json), [release](https://github.com/srivatsav-kannan/solar-seg/releases/tag/batch-dice-v0.3).
...
## Wider context and tiled inference: rejected screens

The frozen `configs/context-screen-03.json` comparison initialized both models from the same 399-image batch-Dice checkpoint and added exactly 2,000 updates with matched crops, sampling, loss, optimizer, and seed. The candidate adds a residual bottleneck with sequential dilations 2/4/8 and a zero-initialized output projection. Tests verify exact parent predictions at initialization and nonzero learning gradients. The baseline implementation remains unchanged; the isolated experimental training and inference modules preserve its recorded behavior.

| Calibration candidate | Pooled PQ | Decision |
|---|---:|---|
| Existing batch-Dice model, full frame | 0.34439 | Reference |
| Additional-training control | 0.34662 | Gain below promotion threshold |
| Additional training plus context branch | 0.34645 | Does not beat matched control |
| Existing batch-Dice model, overlapping 384-pixel tiles | 0.34673 | Gain below promotion threshold |

The context/control paired gain was -0.000172, temporal-bootstrap interval [-0.006314, 0.005756]. Both training runs completed: control 342.7 seconds, context 361.8 seconds on MPS, excluding cache preparation. All calibration runs used four-flip averaging and the same 25-setting reconstruction grid. The tile screen was separately frozen before inference. No candidate advanced to further outer evaluation, test inference, or submission. These exploratory results do not establish that wider context or tiling cannot help other configurations. Public best remains 0.31. Validation: 26 tests passed, including local organizer-evaluator parity; Ruff passed.

[Context evidence](../reports/context-screen-03.json), [tile evidence](../reports/batch-dice-tiled-screen-03.json), [context model](../research/context.py), [training](../research/context_train.py), [calibration](../research/context_calibrate.py).
