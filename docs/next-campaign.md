...
The first exploratory native-resolution screen, frozen in `configs/native-screen.json`, is complete and rejected: full-frame calibration PQ was 0.28188 and tiled PQ was 0.29370, below the selected baseline's 0.33137. It used 2048-pixel images, the same 384-pixel crops, width 24, 6,000 updates, and full-frame versus 384-pixel tiled inference without TTA. This changes linear sampling resolution at matched crop dimensions/update count, while halving physical context per crop; the larger 512-pixel context variant remains a later experiment. Prioritize the context-plus-detail comparison before committing further compute to native resolution alone. No holdout or test inference was run for this rejected model.
